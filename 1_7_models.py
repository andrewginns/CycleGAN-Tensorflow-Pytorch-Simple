...
batch_norm = functools.partial(slim.batch_norm, scale=True, decay=0.9, epsilon=1e-5, updates_collections=None)
...
def generator(img, scope, dim=64, train=True):
    bn = functools.partial(batch_norm, is_training=train)
    conv_bn_relu = functools.partial(conv, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
    deconv_bn_relu = functools.partial(deconv, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
